...
def get_args_parser():
    ...
    parser.add_argument(
        '--output_dir',
        # default=os.path.join(project_path, 'output/log/0722/'),
        default=os.path.join(project_path, os.path.join(project_path, 'log/0812-3/')),
        help='path where to save, empty for no saving'
    )
    parser.add_argument(
        '--resume',
        default=None,
        # default='./output/log/0722/adrs_model_ep0.pth',
        # default=os.path.join(project_path, os.path.join(project_path, 'log/0805-7/checkpoint.pth')),
    ...
    parser.add_argument(
        '--bert_path',
        type=str,
        # default='/mnt/home/xxx/models/bert/bert_uncased_L-2_H-128_A-2',
    ...
    # Subgroup parameters
    parser.add_argument("--use_gender", default=False, type=bool, help='using gender info')
    ...
    args = parser.parse_args()  # Additional namespaces can be incorporated for integration
...
if __name__ == "__main__":
    ...
    # evaluation
    result = evaluation(model, data_loader_val, args=args, is_multi_drug=False, logger=logger)
    result_md = evaluation(model, data_loader_val_md, args=args, is_multi_drug=True, logger=logger)
    if not args.eval:
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            result = evaluation(model, data_loader_val, args=args, is_multi_drug=False, epoch=epoch, logger=logger)
            result_md = evaluation(model, data_loader_val_md, args=args, is_multi_drug=True, epoch=epoch, logger=logger)
